class Solution:
    def canCompleteCircuit(self, gas: List[int], cost: List[int]) -> int:
        ...
        while start < len(gas):
            ...
            curr = 0
            # why does it work without path???
            # we already ensured that sum(gas) > sum(cost)
            # we also know that the prev ones all 
            # the sum(cost) > sum(gas)
            # this means that after the last station
            # the surplus > sum(cost) for the prev stations
            # since we know overall sum(gas) > sum(cost)
